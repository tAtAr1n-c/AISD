...
public class AddOnlyBenchmark {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Path outputTxt = args.length > 1 ? Paths.get(args[1]) : datasetDir.resolve("add_benchmark_results.txt");
    ...
    private static Stat benchmarkOne(String datasetName, List<Integer> data, int warmupRounds, int repeats, int batch) {
        ...
        for (int i = 0; i < repeats; i++) {
            long ns = measureAddNs(data, batch);
            samplesNs.add(ns);
            sum += ns;
            if (ns < min) {
                min = ns;
    ...
    private static long measureAddNs(List<Integer> data, int batch) {
    ...
    private static void writeResults(Path outputTxt, List<Stat> stats, int warmupRounds, int repeats, int batch)
        ...
        try (PrintWriter out = new PrintWriter(Files.newBufferedWriter(outputTxt))) {
            out.println("# FibonacciHeap add benchmark (stable mode)");
