...
public class SearchOnlyBenchmark {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Path outputTxt = args.length > 1 ? Paths.get(args[1]) : datasetDir.resolve("search_benchmark_results.txt");
    ...
    private static Stat benchmarkOne(String datasetName, List<Integer> data, int warmupRounds, int repeats, int batch) {
        ...
        for (int i = 0; i < repeats; i++) {
            long ns = measureSearchNs(data, batch);
            samplesNs.add(ns);
            sum += ns;
            if (ns < min) {
                min = ns;
    ...
    private static long measureSearchNs(List<Integer> data, int batch) {
    ...
    private static void writeResults(Path outputTxt, List<Stat> stats, int warmupRounds, int repeats, int batch)
        ...
        try (PrintWriter out = new PrintWriter(Files.newBufferedWriter(outputTxt))) {
            out.println("# FibonacciHeap search benchmark (stable mode)");
            out.printf("# warmup_rounds=%d repeats=%d batch=%d%n", warmupRounds, repeats, batch);
            out.println("# note: one search averaged over batch queries (50% hits / 50% misses)");
